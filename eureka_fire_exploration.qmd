...
veg_data <- st_read(dsn = "jotrgeodata.gpkg",layer = "JOTR_VegPolys", 
                    quiet = TRUE)

# Read fire severity (raster)
rbr_rast <- raster("refined_rbr.tif")    
```
...
polys <- rasterToPolygons(rbr_to_vec_crs, fun = function(x) !is.na(x) & x != 0, 
                          dissolve = TRUE)
...
ggplot(clipped_joined) +
  ...
  labs(title   = "", caption = sprintf("Total Burned Area: %.1f ha", 
                                       total_fire_ha)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        plot.caption = element_text(hjust  = 2.5, csize = 10, cface = "bold", margin = margin(b = 2)))
```
...
fire_hist <- st_read("HistFires_JOTR_MOJA/FindExistingLocationsOutput.shp")
...
ggplot(all_vals_ordered, aes(x= value, y    = MapUnit_Name, fill = FIRE_NAME)) +
  ...
  scale_x_continuous(name = " Relative Burn Ratio (RBR)", 
                     breaks = seq(-0.2, 0.6, by = 0.2), limits = c(-0.25, 0.65)) +
