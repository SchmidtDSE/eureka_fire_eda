...
veg_data <- st_read(
  dsn   = "/Volumes/Data/GitRepos/eureka_fire_eda/jotrgeodata.gpkg",
  layer = "JOTR_VegPolys",
  quiet = TRUE   
)

# Read fire severity (raster)
rbr_rast <- raster("/Volumes/Data/GitRepos/eureka_fire_eda/refined_rbr.tif")    

#Check coordinate systems 
# crs(veg_data) # NAD83/ UTM zone 11N
# crs(rbr_rast) # WGS 84
```
...
pal      <- colorNumeric("plasma", domain = values(rbr_rast), na.color = "transparent")
pal_rev  <- colorNumeric("plasma", domain = values(rbr_rast), na.color = "transparent", reverse = TRUE)

# Draw the map
leaflet(options = leafletOptions(attributionControl = TRUE)) %>%
  addProviderTiles("CartoDB.Positron") %>%
  
  # Use the reversed palette for the raster itself
  addRasterImage(
    rbr_rast,
    colors  = pal_rev,
    opacity = 0.6,
  ...
  # Use the normal palette + label transform for the legend
  addLegend(
    position  = "bottomright",
    pal       = pal,
    values    = values(rbr_rast),
    title     = "RBR value",
...
ggplot(rbr_df, aes(x = x, y = y, fill = RBR)) +
  ...
  scale_fill_viridis_c(
    option    = "plasma",
    direction = -1,            # so high values are darker purple
  ...
  coord_equal() +
  labs(
    title = "Refined Relative Burn Ratio (RBR)",
    x     = NULL,
    y     = NULL
  ) +
...
rbr_to_vec_crs <- projectRaster(
  rbr_rast,
  crs    = vec_crs,
  method = "bilinear"      
)

# crs(rbr_to_vec_crs) # NAD83/ UTM zone 11N
```
...
# Polygonize all non‐zero, non‐NA cells from the reprojected raster
...
# Convert to sf
fire_polygons <- st_as_sf(polys)

#  merge into a single boundary polygon
fire_boundary <- st_union(fire_polygons) %>% 
  st_sf(geometry = ., crs = st_crs(fire_polygons))

# Write out your new boundary
# st_write(
#   fire_boundary,
#   "generated_fire_boundary_Eureka.gpkg",
#   layer        = "fire_boundary",
#   driver       = "GPKG",
#   delete_layer = TRUE
# )
...
clipped_veg <- clipped_veg %>%
  mutate(
    area_ha = as.numeric(st_area(.) / 10000)  # m² → ha
  )
...
veg_summary <- veg_summary %>%
  mutate(
    pct_of_total = 100 * veg_ha / total_ha
  )
...
# write out
# write_csv(veg_summary, "veg_burned_summary.csv")
...
clipped_joined <- clipped_veg %>%
    dplyr::left_join(
      dplyr::select(veg_summary, MapUnit_Name, veg_ha, pct_of_total),
    by = "MapUnit_Name"
  )
...
legend_labels <- veg_summary %>%
  filter(MapUnit_Name != "Total Burned Area") %>%
  mutate(
    # remove “Association” and clean up whitespace
    clean_name = MapUnit_Name %>%
...
ggplot(clipped_joined) +
  geom_sf(
    aes(fill = factor(MapUnit_Name, levels = ordered_types)),
  ...
  labs(
    title   = "",
    caption = sprintf("Total Burned Area: %.1f ha", total_fire_ha)
  ) +
...
clipped_stats <- clipped_veg %>%
  mutate(
    min_RBR    = exact_extract(rbr_to_vec_crs, ., "min",progress = FALSE),
...
severity_summary <- clipped_stats %>%
  ...
  group_by(MapUnit_Name) %>%
  summarise(
    n_pixels   = sum(n_pixels,     na.rm = TRUE,progress = FALSE),
...
# Build long tibble
vals_df <- tibble(
  MapUnit_Name = clipped_veg$MapUnit_Name,
  value        = vals_list
) %>%
  unnest(cols = c(value)) %>%
  filter(!is.na(value))
...
fire_hist <- st_read("/Volumes/Data/GitRepos/eureka_fire_eda/HistFires_JOTR_MOJA/FindExistingLocationsOutput.shp")
...
pal_fire        <- setNames(viridis(n = length(fire_years), option = "turbo"),
                       fire_years)
...
# plot
ggplot() +
  geom_sf(data = fire_hist,
          aes(fill = factor(YEAR_)),
          colour = "black", size = 0.2, alpha = 0.6) +
  geom_sf(data = fire_boundary,
          fill   = NA, colour = "red", size = 1) +
  
  # zoom to the buffered polygon’s bbox
  coord_sf(xlim = st_bbox(fb)[c("xmin", "xmax")],
           ylim = st_bbox(fb)[c("ymin", "ymax")],
           expand = FALSE) +
  scale_fill_manual(
    name   = "Fire Year",
    values = pal_fire,
...
full_fire_history_table <- full_fire_history %>%
  ...
  mutate(
    YEAR_     = if_else(is.na(YEAR_), 0L, YEAR_),
...
## Summarize statistics fire severity by vegetation type & fire history 
...
full_veg_history <- bind_rows(veg_hist_summary, veg_unburned_summary) %>%
  mutate(
    MapUnit_Name = factor(MapUnit_Name, levels = ordered_veg, ordered = TRUE),
...
all_vals_ordered <- all_vals %>%
  mutate(
    MapUnit_Name = str_remove_all(MapUnit_Name, "Association"),
...
# build legend labels
legend_labs <- c(
  "WHISPERING PINES"    = sprintf("Whispering Pines (19 yrs)"),
  "COVINGTON"           = sprintf("Covington (30 yrs)"),
  "Previously Unburned" = "Unburned (>55 yrs)"
)

ggplot(all_vals_ordered,
       aes(
         x    = value,
         y    = MapUnit_Name,      # <-- make sure y is here
         fill = FIRE_NAME
       )) +
  geom_density_ridges(
    scale           = 1.0,
  ...
  coord_flip() +
  scale_x_continuous(
    name   = " Relative Burn Ratio (RBR)",
    breaks = seq(-0.2, 0.6, by = 0.2),
    limits = c(-0.25, 0.65)
  ) +
  scale_y_discrete(
    name   = NULL,
    labels = function(x) str_wrap(x, 25)
  ) +
  scale_fill_viridis_d(
    name    = "Fire History",
    option  = "magma",
    begin   = 0.2,   # cut off extreme ends if you want
    end     = 0.8,
    labels  = legend_labs
  ) +
  labs(
    title = "Fire Severity Distribution by Vegetation Type × Fire History"
  ) +
  theme_minimal(base_size = 12) +
  theme(
